...
public class AnalyticsExpenseFragment extends BaseFragment {
    ...
    private void PrepareDataForChart() {
        ...
        this.W = myDatabaseHandler.existsColumnInTable();
        this.yvalues = new ArrayList<>();
        this.xVals = new ArrayList<>();
        this.colors = new ArrayList<>();
        this.barEntries = new ArrayList<>();
        if (this.W.booleanValue()) {
            this.X = myDatabaseHandler.getCategoryList(1);
        } else {
            myDatabaseHandler.addcolumn();
        }
        for (int i = 0; i < this.X.size(); i++) {
            CategoryBean categoryBean = this.X.get(i);
            categoryBean.setCategoryTotal(myDatabaseHandler.getCategoryAmount(categoryBean.getCategoryGroup(), categoryBean.getId()));
        }
        this.expenseName = new String[this.X.size()];
        for (int i2 = 0; i2 < this.X.size(); i2++) {
            this.expenseName[i2] = this.X.get(i2).getName();
            this.yvalues.add(new PieEntry(Float.parseFloat(this.X.get(i2).getCategoryTotal()), this.X.get(i2).getName()));
            this.xVals.add(this.X.get(i2).getName());
            this.colors.add(Integer.valueOf(Color.parseColor(this.X.get(i2).getColor())));
            this.barEntries.add(new BarEntry((float) i2, Float.parseFloat(this.X.get(i2).getCategoryTotal())));
        }
        try {
    ...
    private void setPieChart() {
        ...
        this.pieChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry entry, Highlight highlight) {
                ...
                if (AnalyticsExpenseFragment.this.i0().equals("¢") || AnalyticsExpenseFragment.this.i0().equals("₣") || AnalyticsExpenseFragment.this.i0().equals("₧") || AnalyticsExpenseFragment.this.i0().equals("﷼") || AnalyticsExpenseFragment.this.i0().equals("₨")) {
                    str = String.format("%.2f", new Object[]{Double.valueOf(value)}) + AnalyticsExpenseFragment.this.i0();
                } else {
                    str = AnalyticsExpenseFragment.this.i0() + String.format("%.2f", new Object[]{Double.valueOf(value)});
                }
